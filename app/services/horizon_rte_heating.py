...
from ..horizon_cold_schemas import HorizonRteHeatingLoadBackfillRequest
...
from .horizon_cold_regions import HorizonRegionalColdService
from .horizon_cold_response import HorizonColdResponseLibraryService
from .horizon_coverage import HorizonHistoricalCoverageService
from .horizon_rte import (
    RTE_AVAILABLE_FROM,
    RTE_REGIONAL_DATASET,
    RTE_REGIONAL_ENDPOINT,
    HorizonRteCoolingLoadBackfillService,
    _event_region_code,
    _parse_datetime,
    _utc_naive,
    _weekday_class,
...
class HorizonRteHeatingLoadBackfillService:
    ENGINE_VERSION = "horizon-rte-heating-load-backfill-v0.1"
    ADAPTER_KIND = "odre_eco2mix_regional_heating_v1"
    ...
    def _daily_metrics(
        records: list[dict],
        request: HorizonRteHeatingLoadBackfillRequest,
    ) -> dict[date, dict]:
    ...
    def backfill(
        ...
        *,
        ...
        if end_at > datetime.utcnow() + timedelta(minutes=5):
            raise ValueError("historical RTE heating backfill end_at cannot be in the future")
        if end_at - start_at > timedelta(days=366):
            raise ValueError("one RTE historical heating backfill run is limited to 366 days")

        source = HorizonRteCoolingLoadBackfillService(self.db)._source()
        HorizonColdResponseLibraryService(self.db).sync()
        regionalization = HorizonRegionalColdService(self.db).aggregate(
            start_at=start_at,
            end_at=end_at,
        )
        events = (
            ...
            .filter(
                HorizonGlobalEvent.event_type == "extreme_cold_region",
        ...
        metric_spec_id = sha256_dict({
            ...
            "metric": "regional_daily_load_vs_recent_weekday_class_median",
            "baseline_lookback_days": request.baseline_lookback_days,
            "minimum_daily_points": request.minimum_daily_points,
            ...
            for region_code in region_codes:
                ...
                for day in sorted(day for day in daily if start_at.date() <= day <= end_at.date()):
                    ...
                    observation = HorizonObservationIngest(
                        external_key=f"rte-regional-heating:{metric_spec_id}:{region_code}:{day.isoformat()}",
                        observation_type="regional_electricity_heating_outcome",
                        title=f"RTE charge électrique hivernale région {region_code} — {day.isoformat()}",
                        summary=(
                            "Moyenne journalière de consommation électrique régionale comparée à une "
                        ...
                        canonical_facts={
                            ...
                            "daily_mean_consumption_mw": round(metrics["mean_consumption_mw"], 3),
                        ...
                        raw_metadata={
                            ...
                            "metric_spec_id": metric_spec_id,
                            "metric": "00:00-24:00 Europe/Paris mean vs recent weekday/weekend-class median",
                            "heating_causality_proven": False,
                    ...
                    for event in region_events:
                        ...
                        if observed_naive <= event.first_observed_at or observed_naive > outcome_deadline:
                            continue
                        signal_key = f"rte-heating-load:{metric_spec_id}:{event.id}:{region_code}:{day.isoformat()}"
                        ...
                        signal = HorizonSocialSignal(
                            ...
                            signal_type="heating_load_pressure",
                            ...
                            evidence={
                                ...
                                "heating_causality_proven": False,
                                "interpretation": (
                                    "regional daily electricity-load pressure consistent with increased electric heating demand"
                                ),
                            },
                ...
                coverage = coverage_service.record_interval(
                    ...
                    signal_types=["heating_load_pressure"],
                    geography=["FR", f"REGION:{region_code}"],
                    start_at=start_at,
                    end_at=end_at,
                    completeness="complete" if coverage_complete else "partial",
                    basis="rte_regional_half_hour_consumption_with_daily_baseline_metric_available_each_calendar_day",
                    provenance={
                        "engine": self.ENGINE_VERSION,
                        "dataset_id": RTE_REGIONAL_DATASET,
                        "metric_spec_id": metric_spec_id,
        ...
        result = {
            ...
            "regional_cold_events_considered": len(relevant_events),
            "region_codes": region_codes,
            "raw_outcome_observations_created_or_reused": len(set(observation_ids)),
            "heating_load_signal_links_created_or_reused": len(set(signal_ids)),
            "coverage_interval_ids": coverage_ids,
            "regions": region_results,
            "errors": errors,
            "critical_semantics": {
                "rte_consumption_is_behavioral_outcome_proxy": True,
                "rte_load_proves_electric_heating_causality": False,
                "regional_cold_requires_multiple_departments": True,
                "derived_metric_identity_includes_configuration": True,
                "negative_labels_require_complete_signal_coverage": True,
                "daily_load_metric_is_not_temperature_measurement": True,
