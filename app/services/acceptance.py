...
class AllocationAcceptanceService:
    ...
    def _context(
        ...
        CollectivePrivateAllocation,
        CollectiveAllocationRound,
        CollectiveMarketOffer,
        CollectiveConditionalCommitment,
        CollectiveIntentMembership,
        PrivateIntentEnvelope,
    ]:
        private = (
            self.db.query(CollectivePrivateAllocation)
            .filter(CollectivePrivateAllocation.allocation_entry_id == allocation_entry_id)
            .one_or_none()
        )
    ...
    def decide(
        ...
        decision: str,
        confirm: str,
    ) -> CollectiveAllocationDecision:
        if decision not in {"accepted", "rejected"}:
            raise ValueError("unsupported allocation decision")
        private, round_row, offer, commitment, membership, envelope = self._context(
            user, allocation_entry_id
        )
        ...
        required = (
            f"{verb} ALLOCATION {private.allocation_entry_id} "
            f"{private.allocated_quantity} {round_row.allocation_set_hash[-12:]}"
        )
        if confirm != required:
    ...
    def revoke_acceptance(
        self,
        user: User,
        decision: CollectiveAllocationDecision,
        confirm: str,
    ) -> CollectiveAllocationDecision:
        ...
        commitment = (
            self.db.query(CollectiveConditionalCommitment)
            .filter(CollectiveConditionalCommitment.id == decision.commitment_id)
            .one()
        )
