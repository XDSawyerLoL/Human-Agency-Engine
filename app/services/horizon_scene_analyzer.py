...
class PublicSceneAnalyzer:
    ENGINE_VERSION = "horizon-public-scene-analyzer-v0.1"
    ...
    def camera_registry(self) -> dict[str, Any]:
            ...
            try:
                ...
                for item in payload:
                    try:
                        ...
                        continue
                    if not camera.display_authorized:
                        continue
        ...
        return {
            ...
            "errors": errors,
            "configuration": {
                "environment_variable": "HORIZON_PUBLIC_CAMERAS_JSON",
                "requires_display_authorized": True,
                "analysis_requires_separate_authorization": True,
                "windy_webcams_adapter_ready_for_future_keyed_integration": bool(
                    os.getenv("HORIZON_WINDY_WEBCAMS_API_KEY", "").strip()
                ),
            },
