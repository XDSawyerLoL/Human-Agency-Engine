...
router = APIRouter(prefix="/collective-acceptance", dependencies=[Depends(require_api_key)])
...
def accept_allocation(external_id: str, payload: AllocationDecisionCreate, db: Session = Depends(get_db)):
    user = _user_or_404(db, external_id)
    try:
        row = AllocationAcceptanceService(db).decide(user, payload.allocation_entry_id, "accepted", payload.confirm)
...
def reject_allocation(external_id: str, payload: AllocationDecisionCreate, db: Session = Depends(get_db)):
    user = _user_or_404(db, external_id)
    try:
        row = AllocationAcceptanceService(db).decide(user, payload.allocation_entry_id, "rejected", payload.confirm)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _out(row)
...
def revoke_allocation_acceptance(external_id: str, decision_id: str, payload: AllocationAcceptanceRevoke, db: Session = Depends(get_db)):
    user = _user_or_404(db, external_id)
    row = db.query(CollectiveAllocationDecision).filter(CollectiveAllocationDecision.decision_id == decision_id).one_or_none()
    if not row:
...
def list_user_allocation_decisions(external_id: str, db: Session = Depends(get_db)):
    user = _user_or_404(db, external_id)
    rows = db.query(CollectiveAllocationDecision).filter(CollectiveAllocationDecision.user_id == user.id).order_by(CollectiveAllocationDecision.decided_at.desc()).all()
    return {
