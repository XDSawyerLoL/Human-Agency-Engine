(() => {
  ...
  function fusionCount(f){return Math.max(1,Number(f?.fusion?.raw_forecast_count)||1)}
  ...
  function sourceChips(consolidation){
    const providers=consolidation?.source_providers||[];
    if(providers.length)return providers.map(s=>`<span class="source-chip" title="${esc(s.role||"source")}">${esc(s.label||s.key)}</span>`).join("");
    const families=consolidation?.source_families||[];
    if(!families.length)return`<span class="source-chip">Sources en consolidation</span>`;
    return families.map(s=>`<span class="source-chip" title="Famille de preuve">${esc(s.label||s.key)}</span>`).join("");
  }
  function sourceSummary(consolidation){
    const providers=consolidation?.source_providers||[];
    if(providers.length)return providers.slice(0,3).map(s=>s.label||s.key).join(" · ");
    return (consolidation?.source_families||[]).slice(0,3).map(s=>s.label||s.key).join(" · ")||"Sources en consolidation";
  }
  ...
  function sparkline(forecast){
    ...
    const values=hist.length?hist:[probability(forecast)];if(values.length===1)values.unshift(values[0]);
    ...
    const pts=points.map(([x,y])=>`${x},${y}`).join(" "),[lx,ly]=points.at(-1);
  ...
  function chainHtml(f){
    const steps=(f?.causal_chain||[]).filter(Boolean);if(!steps.length)return`<span>Chaîne encore incomplète</span>`;
    return steps.map((s,i)=>`${i?"<i>→</i>":""}<span>${esc(s)}</span>`).join("");
  }
  function competitionHtml(f){
    const c=f?.consolidation?.scenario_competition;let outcomes=c?.outcomes||[];
  ...
  function divergenceHtml(f){
    const d=f?.consolidation?.divergence||{},delta=Number(d.delta_points),signed=Number.isFinite(delta)?`${delta>0?"+":""}${delta.toFixed(1)} pts`:"—",external=Boolean(d.external_consensus_available);
  ...
  function cardHtml(f){
    ...
    return `<article class="forecast-card"><div><div class="meta">${esc(origin(f))} · ${esc(DOMAIN_LABELS[f.domain]||f.domain_label||f.domain||"Monde")}${count>1?` · ${count} alertes fusionnées`:""}</div><h3>${esc(f.headline||f.outcome||"Scénario")}</h3><div class="card-window">${esc(f?.time_window?.human||"Fenêtre indéterminée")} · ${esc(d.label)}</div></div><div class="mini-ring" style="--p:${p}"><strong>${p}%</strong></div><div class="card-footer"><span>${esc(sourceSummary(c))}</span><b>Consolidation ${c.score??"—"}/100 · ${esc(c.level||"en cours")}</b></div><details><summary>Voir pourquoi cette prévision existe ↓</summary><div>${esc(f.why_now||"Explication non publiée.")}<br><br><b>Invalidation :</b> ${esc(f.falsification||"non publiée")}</div></details></article>`;
  }
