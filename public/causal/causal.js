...
function chainForForecast(f){
  ...
  while(queue.length&&guard<180){guard++;const {edge,depth}=queue.shift();const edgeKey=`${edge.from}->${edge.to}:${edge.type||''}`;if(seenEdges.has(edgeKey))continue;seenEdges.add(edgeKey);const n=node(edge.from);if(!n)continue;
  ...
  return {mechanisms:uniq(mechanisms,5),sources:uniq(sources,5),upstreamForecasts:uniq(upstreamForecasts,3)};
}
