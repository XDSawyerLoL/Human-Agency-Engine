...
function chainForForecast(f){
  const first=incoming(f.id).sort((a,b)=>(Number(b.strength)||0)-(Number(a.strength)||0));
  const mechanisms=[],sources=[];
  for(const e of first){const n=node(e.from);if(!n)continue;if(n.type==='concept'){mechanisms.push({node:n,edge:e});for(const e2 of incoming(n.id)){const s=node(e2.from);if(s?.type==='source')sources.push({node:s,edge:e2});}}else if(n.type==='source')sources.push({node:n,edge:e});}
  const uniq=(rows,limit)=>{const seen=new Set();return rows.filter(x=>!seen.has(x.node.id)&&seen.add(x.node.id)).slice(0,limit)};
  return {mechanisms:uniq(mechanisms,3),sources:uniq(sources,4)};
}
function filteredForecasts(){
  ...
  if(q)rows=rows.filter(n=>String(n.label).toLowerCase().includes(q)||chainForForecast(n).mechanisms.some(x=>String(x.node.label).toLowerCase().includes(q))||chainForForecast(n).sources.some(x=>String(x.node.label).toLowerCase().includes(q)));
  rows.sort((a,b)=>(Number(b.probability_percent)||0)-(Number(a.probability_percent)||0));
  if(domain||q)return rows.slice(0,24);
  const perDomain=new Map(),out=[];for(const f of rows){const n=perDomain.get(f.domain)||0;if(n<2){out.push(f);perDomain.set(f.domain,n+1)}}return out.slice(0,24);
}
