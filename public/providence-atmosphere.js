(()=>{
  ...
  const nav=[
    ['home','/','⌾','Vue d’ensemble'],['predictions','/predictions/','◈','Prédictions'],['horizons','/horizons/','◷','Horizons'],['causal','/causal/','⌘','Causal World'],['sports','/sports/','⚽','Sports Calibration'],['intelligence','/intelligence/','◆','Décision Intelligence'],['track-record','/track-record/','▥','Track Record'],['modules','/modules/','⬡','Modules & Modèles'],['sources','/sources/','≋','Sources & Données'],['cameras','/cameras/','◉','Monde en direct']
  ];
  if(!document.querySelector('.pv12-sidebar')){
    const aside=document.createElement('aside');aside.className='pv12-sidebar';aside.setAttribute('aria-label','Navigation Providence');
    aside.innerHTML=`<a class="pv12-side-brand" href="/"><span class="pv12-side-mark"></span><span><strong>PROVIDENCE</strong><small>VOIR · CALIBRER · COMPRENDRE</small></span></a><nav class="pv12-side-nav">${nav.map(([key,href,icon,label])=>`<a href="${href}" class="${key===page?'active':''}" ${key===page?'aria-current="page"':''}><span class="pv12-side-icon">${icon}</span><span>${label}</span></a>`).join('')}</nav><div class="pv12-mission"><div class="pv12-mission-orb">◉</div><strong>NOTRE MISSION</strong><p>Mesurer la réalité.<br>Améliorer la prédiction.<br>Réduire l’incertitude.</p></div><div class="pv12-side-foot">PROVIDENCE V12 · Forecast Command Center</div>`;
  ...
  const palette={home:[70,184,255],predictions:[112,107,255],horizons:[63,220,255],causal:[160,94,255],sports:[52,169,255],intelligence:[66,226,210],'track-record':[62,175,255],modules:[52,215,255],sources:[82,231,186],cameras:[56,200,255]};const rgb=palette[page]||palette.home;
  function resize(){dpr=Math.min(devicePixelRatio||1,2);w=innerWidth;h=innerHeight;canvas.width=w*dpr;canvas.height=h*dpr;canvas.style.width=`${w}px`;canvas.style.height=`${h}px`;ctx.setTransform(dpr,0,0,dpr,0,0);const n=Math.max(38,Math.min(118,Math.floor(w*h/17000)));points=Array.from({length:n},()=>({x:Math.random()*w,y:Math.random()*h,vx:(Math.random()-.5)*.09,vy:(Math.random()-.5)*.09,r:.45+Math.random()*1.4,p:Math.random()*Math.PI*2}));}
  ...
  function frame(t){ctx.clearRect(0,0,w,h);const glow=ctx.createRadialGradient(w*(.57+(mouse.x-.5)*.07),h*(.16+(mouse.y-.5)*.04),0,w*.55,h*.18,Math.max(w,h)*.68);glow.addColorStop(0,`rgba(${rgb.join(',')},.07)`);glow.addColorStop(.5,'rgba(111,76,220,.025)');glow.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=glow;ctx.fillRect(0,0,w,h);
    for(const p of points){p.x+=p.vx;p.y+=p.vy;if(p.x<-5)p.x=w+5;if(p.x>w+5)p.x=-5;if(p.y<-5)p.y=h+5;if(p.y>h+5)p.y=-5;const pulse=.45+.35*Math.sin(t*.00045+p.p);ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=`rgba(${rgb.join(',')},${.13*pulse})`;ctx.fill();}
    ctx.lineWidth=.5;for(let i=0;i<points.length;i++){for(let j=i+1;j<Math.min(points.length,i+12);j++){const a=points[i],b=points[j],dx=a.x-b.x,dy=a.y-b.y,d=Math.hypot(dx,dy);if(d<128){ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.strokeStyle=`rgba(${rgb.join(',')},${(1-d/128)*.04})`;ctx.stroke();}}}requestAnimationFrame(frame);
