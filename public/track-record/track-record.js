(() => {
  ...
  const pct01 = v => `${Math.round(n(v) * 1000) / 10}%`;
  ...
  function recentRows(rows) {
    ...
    return rows.map(r => `<div class="v5-track-row"><div><b>${esc(r.title || r.scenario_id || 'Scénario')}</b><small>${esc(DOMAIN[r.domain] || r.domain || 'Monde')} · ${esc(HORIZON[r.horizon_tier] || r.horizon_tier || 'horizon variable')}</small></div><span class="pct">${pctProb(r.first_probability)}</span><span class="pct">${pctProb(r.last_probability)}</span><span>${esc(date(r.target_at))}</span></div>`).join('');
  }
  ...
  function buckets(rows) {
    ...
    return active.map(b=>{
      const observed=n(b.observed_frequency); const mean=n(b.mean_probability); const gap=b.calibration_gap===null?'—':`${b.calibration_gap>=0?'+':''}${Math.round(n(b.calibration_gap)*1000)/10} pts`;
      return `<div class="v8-bucket"><small>${esc(b.label)}</small><div class="v8-bucket-track"><i style="--w:${Math.round(observed*100)}%"></i></div><strong>${Math.round(mean*100)} → ${Math.round(observed*100)}%</strong><small style="display:none">${esc(gap)}</small></div>`;
    }).join('');
  }
  ...
  function segmentRows(rows,type) {
    ...
    return rows.map(x=>{
      const label=type==='domain'?(DOMAIN[x.key]||x.key):(HORIZON[x.key]||x.key);
      const state=x.ready?'ready':'collecting';
      return `<div class="v8-segment"><b>${esc(label)}</b><small>${n(x.n)} cas</small><span>Brier ${x.brier===null?'—':Number(x.brier).toFixed(3)}</span><span class="${state}">${x.ready?'mesurable':'collecte'}</span></div>`;
    }).join('');
  }
  ...
  async function init() {
    try {
      ...
      $('#trackNote').textContent = c.calibration_ready
        ? `${n(c.scorable_resolutions)} résolutions binaires entrent maintenant dans le score public.`
        : `${n(c.scorable_resolutions)} résolutions binaires scorables. Le seuil public de calibration est ${n(c.minimum_global_samples)||30}.`;
      ...
      $('#calibrationText').textContent = c.calibration_ready
        ? `Calibration active sur ${n(c.scorable_resolutions)} scénarios binaires résolus. ECE mesure l’écart entre probabilité annoncée et fréquence observée ; le skill compare le Brier à une baseline de fréquence.`
        : `Le moteur calcule déjà les scores, mais ne les utilise pas pour modifier les probabilités publiques avant ${n(c.minimum_global_samples)||30} résolutions binaires vérifiées.`;
      const persistent=d.storage_mode==='mysql'&&d.persistent_learning;
      $('#storageWarning').textContent = persistent
        ? 'Historique persistant actif : probabilités, métadonnées, preuves et résolutions survivent aux redéploiements Hostinger.'
        : 'Apprentissage en mémoire : le moteur fonctionne, mais le corpus peut être perdu à un redémarrage tant que MySQL Hostinger n’est pas raccordé.';
