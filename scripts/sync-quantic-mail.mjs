...
const COMMIT = "d1e1ad51a8686c30677666c659952b4d72cc7491";
...
try {
  ...
  run("npm", ["run", "build:hostinger"], {
    ...
    env: {
      ...process.env,
      NEXT_PUBLIC_QUANTIC_BOOTSTRAPS:
        process.env.NEXT_PUBLIC_QUANTIC_BOOTSTRAPS || DEFAULT_BOOTSTRAPS,
    },
  });

  const built = join(work, "out");
  for (const required of ["index.html", join("network", "index.html"), join("vault", "index.html")]) {
    if (!existsSync(join(built, required))) {
      throw new Error(`QuanticMail export incomplet: ${required} absent.`);
    }
  }
