...
const DOMAIN_HINTS = {
  Technologie:/\b(ai|ia|artificial|technology|technologie|cyber|digital|numerique|regulation|data center|semiconductor|software)\b/,
  Climat:/\b(weather|climat|climate|flood|heat|storm|drought|wildfire|temperature|co2|emission|hurricane|rain|wind)\b/,
  Emploi:/\b(layoff|labor|labour|emploi|industrial|unemployment|chomage|worker|workforce|job|factory)\b/,
  Énergie:/\b(energy|energie|oil|petrole|fred|grid|electric|electricity|nuclear|carbon|ets|power)\b/,
  Santé:/\b(health|who|oms|disease|outbreak|sante|medical|clinical|cancer|infectious|epidemi)\b/,
  Société:/\b(strike|greve|protest|social|societe|worker|collective|peace|population)\b/,
  Géopolitique:/\b(conflict|military|sanction|geopolit|trade|war|naval|china|chine|ukraine|taiwan|security)\b/,
  Économie:/\b(bank|banque|inflation|cpi|recession|credit|finance|fred|rate|taux|gdp|pib|liquidity|liquidite)\b/
};
...
function matchSeed(seed, signals){
  ...
  const domainHint=DOMAIN_HINTS[seed.domain];
  const scored=[];
  for(const signal of signals||[]){
    ...
    const regionBoost=region&&region!=='monde'&&text.includes(region)?0.26:0;
    const sourceBoost=sourceTokens.some(s=>s&&text.includes(s))?0.16:0;
    const domainBoost=domainHint?.test(text)?0.14:0;
    const score=lexical+regionBoost+sourceBoost+domainBoost;
    if(score>=0.12)scored.push({signal,score});
  }
...
function probability(seed,matches){
  ...
  // Future Engine is our own scenario memory. Keep a discounted prior and let present signals move it.
  const retained=.38+confidence*.34;
  ...
  for(const {signal,score} of matches){
    ...
    support += clamp(score,0,.9)*trust*(duplicate?.35:1)*.64;
  }
  z += clamp(support,0,1.45);
  if(!matches.length) z -= .12;
...
function eventType(seed){
  ...
  if(seed.domain==='Géopolitique'&&/naval|chine|china|taiwan|military|conflit|war/.test(title))return'memory_geopolitical_escalation';
  if(seed.domain==='Économie'&&/banque centrale|inflation|cpi|recession|rate|taux/.test(title))return'memory_macro_transition';
...
export function buildScenarioMemoryForecasts(signals,{now=Date.now()}={}){
  ...
  for(const seed of catalog){
    ...
    const declared=(seed.sources||[]).map(label=>({key:sourceKey(label),label,role:'source du scénario initial'}));
    out.push({
      ...
      probability:{type:'model_estimate',estimate:p,percent:pct,interval_low:clamp(p-.16,.02,.88),interval_high:clamp(p+.16,.10,.95),interval_percent:[Math.round(clamp(p-.16,.02,.88)*100),Math.round(clamp(p+.16,.10,.95)*100)],method:'evidence-scenario-memory-v2',calibration_status:'uncalibrated_model_estimate',empirically_calibrated:false,can_be_read_as_empirical_frequency:false},
      ...
      what_we_know:matches.length?`${matches.length} signaux HORIZON actuels recoupent cette hypothèse.`:'Cette hypothèse issue de notre moteur Future Engine reste sous surveillance active par HORIZON.',
      why_now:matches.length?`HORIZON retrouve actuellement ${matches.length} recoupement(s) utile(s), issus de ${Math.max(1,independentFamilies)} famille(s) de sources.`:'Le moteur conserve cette hypothèse en mémoire et attend des précurseurs actuels suffisamment proches pour la renforcer ou l’affaiblir.',
      causal_chain:['hypothèse issue de la mémoire prédictive','recherche de précurseurs actuels','corroboration indépendante','matérialisation ou réfutation avant échéance'],
      ...
      evidence,
      fusion:{engine:'evidence-scenario-memory-v2',raw_signal_count:evidence.length,source_keys:evidence.map(x=>x.source_key),duplicate_probability_inflation_prevented:true,geography_aware_grouping:true,probability_recomputed_after_fusion:true,multiple_distinct_outcomes_per_precursor_allowed:true},
      consolidation:{score:Math.round(clamp(Number(seed.confidence||35)*.55+independentFamilies*8+Math.min(matches.length,4)*4,30,92)),score_is_probability:false,level:matches.length?'en consolidation':'surveillance',source_families:[...new Map(evidence.map(x=>[x.source_family,{key:x.source_family,label:x.source_family}])).values()],source_providers:[...declared,...evidence.map(x=>({key:x.source_key,label:x.source_label||x.source_key,role:x.source_family||'signal HORIZON'}))].filter((x,i,a)=>a.findIndex(y=>y.key===x.key)===i),dimensions:[],strengths:[`Mémoire prédictive : estimation initiale ${seed.probability}%.`,matches.length?`${matches.length} recoupement(s) actuel(s) détecté(s).`:'Hypothèse explicite avec échéance.'],weaknesses:matches.length?[]:['Pas encore de recoupement live assez fort.']},
      novelty:'scenario_memory_reforecast',commercial_contract:{certainty_claimed:false,falsifiable:true,expiry_enforced:true},
...
export function scenarioMemorySeeds({activeOnly=true,source=''}={}){
  const needle=NORMALIZE(source);
  return catalog.filter(seed=>{
    if(activeOnly&&Date.parse(seed.target_date)<Date.now())return false;
    if(needle&&!(seed.sources||[]).some(s=>NORMALIZE(s).includes(needle)))return false;
    return true;
