...
const DOMAIN_CAPS = {
  natural_hazards:3, weather_climate:3, public_health:7, cyber_technology:8, financial_stress:7,
  energy:7, economy_labor:8, supply_fuel:8, social_collective_behavior:6, geopolitics_security:8,
  regulation_policy:7, transport_mobility:5
};
const HORIZON_CAPS = { immediate:10, near:16, medium:18, long:14, strategic:10, deep:8 };
...
export function selectPublicForecasts(rows, limit=60){
  ...
  const canAdd=(row, relaxed=false)=>{
    ...
    if(dc >= (DOMAIN_CAPS[row.domain] ?? 6)) return false;
    if(hc >= (HORIZON_CAPS[row.horizon_tier] ?? 10)) return false;
    if(oc >= 2) return false;
    if(ec >= (relaxed ? 4 : 2)) return false;
  ...
  // Pass 1: breadth first — strongest scenario available for every domain.
  ...
  // Pass 2: every horizon gets a representative when a candidate exists.
  ...
  // Pass 3: fill with best remaining futures under strict diversity caps.
  for(const row of unique){
    if(selected.length>=limit) break;
    if(canAdd(row)) add(row);
  }

  // Pass 4: relax event-family count only. Never relax semantic dedup or environmental cap.
  for(const row of unique){
    if(selected.length>=limit) break;
